...
contract("MyntToken", async (accounts) => {
    ...
    describe("setPresale", async () => {
        context("should fail", async () => {
            it("when it's not called by owner", async () => {
                await expectRevert(token.setPresale(user, { from: user }), NOT_OWNER_EXCEPTION);
            });
        });
    ...
    describe("setMarketMaker", async () => {
        context("should fail", async () => {
            it("when it's not called by owner", async () => {
                await expectRevert(token.setMarketMaker(user, { from: user }), NOT_OWNER_EXCEPTION);
            });
        });
        context("should succeed", async () => {
            it("when called by owner", async () => {
    ...
    describe("mint", async () => {
        context("should fail", async () => {
            it("when it's not called by presale or market maker", async () => {
                await expectRevert(token.mint(user, toWei("100"), { from: user }), "not allowed");
            });
        });
        ...
        context("should succeed", async () => {
            it("when it's called by presale", async () => {
                ...
                expectEvent(tx, "Transfer", { from: ZERO_ADDRESS, to: user, value: toWei("100") });
            });
    ...
    describe("burn", async () => {
        context("should fail", async () => {
            it("when it's not called by presale or by a user", async () => {
                await expectRevert(token.burn(user, toWei("50"), { from: owner }), "not allowed");
            });
        });
        ...
        context("should succeed", async () => {
            it("when it's called by the market maker", async () => {
                ...
                expectEvent(tx, "Transfer", { from: user, to: ZERO_ADDRESS, value: toWei("50") });
            });
    ...
    describe("approveAndCall", async () => {
        ...
        it("should approve for transfer and call the receiver", async () => {
            const tx = await token.approveAndCall(approvalReceiver.address, toWei("50"), "0x1234");
            await expectEvent(tx, "Approval", { owner, spender: approvalReceiver.address, value: toWei("50") });
            expect(await approvalReceiver.sender(), "sender").eq(owner);
            expect((await approvalReceiver.amount()).toString(), "amount").eq(toWei("50"));
