    ...
    describe("withdraw", async () => {
        ...
        context("should succeed", async () => {
            it("when vault balance is sufficient", async () => {
                ...
                await feesVault.withdraw(token1.address, tokens(5), user2, { from: owner });
                expect(await token1.balanceOf(user2)).bignumber.to.eq(tokens(5), "tokens should be transfered");
            });
        });
