...
contract("InitializableAdminUpgradeabilityProxy", async accounts => {
  ...
  describe("changeAdmin", async () => {
    ...
    context("should fail", async () => {
      it("when caller is not admin", async () => {
        await expectRevert(
          adminUpgradeabilityProxy.changeAdmin(sa.governor, {
            from: sa.dummy1
          }),
    ...
    context("should succeed", async () => {
      it("when all params are valid", async () => {
        ...
        const tx = await adminUpgradeabilityProxy.changeAdmin(newAdmin, {
          from: admin
        });
  ...
  describe("integration", async () => {
    ...
    it("full flow test", async () => {
      ...
      await adminUpgradeabilityProxy.methods[
        "initialize(address,address,bytes)"
      ](proxyImplementation.address, sa.governor, initdata);
      ...
      expect(isImplementationInitalized).to.eq(
        true,
      ...
      await adminUpgradeabilityProxy.changeAdmin(sa.default, {
        from: sa.governor
      });
