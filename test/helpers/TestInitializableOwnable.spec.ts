...
const InitializableOwnableWrapper = artifacts.require(
...
contract("InitializableOwnable", async accounts => {
  ...
  describe("renounceOwnership", async () => {
    ...
    it("should properly clear owner state", async () => {
      ...
      let isCurrOwner = await initializableOwnableWrapper.isOwner({
        from: owner
      });
  ...
  describe("transferOwnership", async () => {
    ...
    context("should succeed", async () => {
      it("when it's called by owner", async () => {
        ...
        await initializableOwnableWrapper.transferOwnership(user, {
          from: owner
        });
        const currOwner = await initializableOwnableWrapper.owner({
          from: owner
    ...
    context("should fail", async () => {
      ...
      it("when new owner is zero address", async () => {
        ...
        await expectRevert(
          initializableOwnableWrapper.transferOwnership(ZERO_ADDRESS, {
            from: owner
          }),
