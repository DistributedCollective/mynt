...
const InitializableReentrancyMock = artifacts.require(
  ...
  describe("initialize", async () => {
    ...
    context("should succeed", async () => {
      it("without reentrancy", async () => {
        ...
        await initializableReentrancyMock.runClientMethod(
          nonReentrantMock.address,
          {
            from: owner
          }
        );
    ...
    context("should fail", async () => {
      ...
      it("in case of reentrancy", async () => {
        const reentrantMock = await ReentrantMock.new(
          initializableReentrancyMock.address
        );
      ...
      it("when it's not initialized", async () => {
        ...
        await expectRevert(
          initializableReentrancyMock.runClientMethod(
            nonReentrantMock.address,
            {
              from: owner
            }
