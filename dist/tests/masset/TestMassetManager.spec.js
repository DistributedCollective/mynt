...
contract("MassetManager", async (accounts) => {
    ...
    describe("initialize", async () => {
        ...
        beforeEach(async () => {
            ...
            basketManagerObj = await createBasketManager(massetManager, [18, 18], [1, 1]);
            mAsset = await createMassetToken(massetManager);
        });
        context("should succeed", async () => {
            it("when given a valid basket manager", async () => {
                await massetManager.initialize(basketManagerObj.basketManager.address, mAsset.address, false);
                ...
                // migrate to V3
                await massetManager.upgradeToV3(basketManagerObj.basketManager.address, mAsset.address, vault.address, feesManager.address);
        ...
        context("should fail", async () => {
            it("when basket manager missing", async () => {
                await (0, test_helpers_1.expectRevert)(massetManager.initialize(constants_1.ZERO_ADDRESS, mAsset.address, false), "VM Exception while processing transaction: reverted with reason string 'invalid basket manager'");
            });
            it("when token missing", async () => {
                await (0, test_helpers_1.expectRevert)(massetManager.initialize(basketManagerObj.basketManager.address, constants_1.ZERO_ADDRESS, false), "VM Exception while processing transaction: reverted with reason string 'invalid mAssetToken'");
            });
            it("when already initialized", async () => {
                await massetManager.initialize(basketManagerObj.basketManager.address, mAsset.address, false);
                await (0, test_helpers_1.expectRevert)(massetManager.initialize(basketManagerObj.basketManager.address, mAsset.address, false), "VM Exception while processing transaction: reverted with reason string 'Initializable: contract is already initialized'");
            });
        });
    });
    describe("mint", async () => {
        beforeEach(async () => {
            ...
            vault = await FeesVault.new();
            mAsset = await createMassetToken(massetManager);
            basketManagerObj = await createBasketManager(massetManager, [18, 18], [100, 1]);
            await initMassetManager(massetManager, basketManagerObj.basketManager.address, mAsset.address, vault.address, standardFees);
        ...
        context("should succeed", () => {
            it("if all params are valid", async () => {
                const sum = (0, tools_1.tokens)(1024);
                const massetQuantity = sum.div(new tools_1.BN(100));
                ...
                const tx = await massetManager.mint(basketManagerObj.mockToken1.address, sum, {
                    from: standardAccounts.dummy1,
                });
        ...
        context("should fail", () => {
            it("if basset is invalid", async () => {
                await (0, test_helpers_1.expectRevert)(massetManager.mint(constants_1.ZERO_ADDRESS, 10), "VM Exception while processing transaction: reverted with reason string 'invalid basset'");
            });
            it("if basset is not in the basket", async () => {
                await (0, test_helpers_1.expectRevert)(massetManager.mint(mockTokenDummy.address, 10), "VM Exception while processing transaction: reverted with reason string 'invalid basset'");
            });
            it("if amount is greater than the balance", async () => {
                await basketManagerObj.mockToken1.approve(massetManager.address, 100000);
                await (0, test_helpers_1.expectRevert)(massetManager.mint(basketManagerObj.mockToken1.address, 100000), "VM Exception while processing transaction: reverted with reason string 'ERC20: transfer amount exceeds balance'");
            });
        });
    });
    describe("mintTo", async () => {
        ...
        context("should succeed", () => {
            it("if all params are valid", async () => {
                const sum = (0, tools_1.tokens)(1);
                const expectedFee = sum.mul(standardFees.deposit).div(constants_1.FEE_PRECISION);
                const expectedMassetQuantity = sum.sub(expectedFee);
                await basketManagerObj.mockToken1.approve(massetManager.address, sum, {
                    from: standardAccounts.dummy1,
                });
                const tx = await massetManager.mintTo(basketManagerObj.mockToken1.address, sum, standardAccounts.dummy4, {
                    from: standardAccounts.dummy1,
                });
                await (0, test_helpers_1.expectEvent)(tx.receipt, "Minted", {
                    minter: standardAccounts.dummy1,
    ...
    describe("redeem", async () => {
        ...
        context("should succeed", () => {
            it("if all params are valid", async () => {
                ...
                const sum = new tools_1.BN(123123).pow(new tools_1.BN(2));
                const mintFee = sum.mul(standardFees.deposit).div(constants_1.FEE_PRECISION);
                await basketManagerObj.mockToken1.approve(massetManager.address, sum, {
                    from: standardAccounts.dummy1,
                });
                ...
                const tx = await massetManager.redeem(basketManagerObj.mockToken1.address, mintedMassets, {
                    from: standardAccounts.dummy1,
                });
                ...
                expect(balance).bignumber.to.equal((0, tools_1.tokens)(0));
                balance = await basketManagerObj.mockToken1.balanceOf(standardAccounts.dummy1);
            ...
            it("if all params are valid, amounts that don't divide evenly", async () => {
                const factor = new tools_1.BN(-10);
                await basketManagerObj.basketManager.setFactor(basketManagerObj.mockToken1.address, factor);
                const initialBalance = await basketManagerObj.mockToken1.balanceOf(standardAccounts.dummy1);
                const sum = new tools_1.BN(123123);
                ...
                expect(sumOfOperations).bignumber.to.eq(sum, "check that sum of funds in system after deposit and redeem is the same as before");
                const vaultBalance = await mAsset.balanceOf(vault.address);
                expect(vaultBalance).bignumber.to.eq(mintFee.add(withdrawalFee));
            });
        });
        context("should fail", () => {
            it("if basset is invalid", async () => {
                await (0, test_helpers_1.expectRevert)(massetManager.redeem(constants_1.ZERO_ADDRESS, 10), "VM Exception while processing transaction: reverted with reason string 'invalid basset'");
            });
            it("if basset is not in the basket", async () => {
                await (0, test_helpers_1.expectRevert)(massetManager.redeem(mockTokenDummy.address, 10), "VM Exception while processing transaction: reverted with reason string 'invalid basset'");
            });
            it("if amount is greater than the balance", async () => {
                ...
                await (0, test_helpers_1.expectRevert)(massetManager.redeem(basketManagerObj.mockToken1.address, 1000, {
                    from: standardAccounts.dummy1,
            ...
            it("if amount is greater than balance", async () => {
                ...
                await (0, test_helpers_1.expectRevert)(
    ...
    describe("redeemTo", async () => {
        ...
        context("should fail", () => {
            it("when recipient is not valid", async () => {
                await (0, test_helpers_1.expectRevert)(massetManager.redeemTo(basketManagerObj.mockToken1.address, (0, tools_1.tokens)(10), constants_1.ZERO_ADDRESS, { from: standardAccounts.dummy1 }), "VM Exception while processing transaction: reverted with reason string 'must be a valid recipient'");
            });
        });
        context("should succeed", () => {
            it("if all params are valid", async () => {
                const sum = new tools_1.BN(10).pow(new tools_1.BN(3));
                const mintFee = sum.mul(standardFees.deposit).div(constants_1.FEE_PRECISION);
                ...
                await massetManager.redeemTo(basketManagerObj.mockToken1.address, withdrawAmount, recipient, { from: standardAccounts.dummy1 });
    ...
    describe("redeemToBridge", async () => {
        const mintAmount = (0, tools_1.tokens)(1);
        const mintFee = mintAmount.mul(standardFees.deposit).div(constants_1.FEE_PRECISION);
        ...
        beforeEach(async () => {
            ...
            basketManagerObj = await createBasketManager(massetManager, [18, 18], [1, 1], [mockBridge.address, mockBridge.address]);
            await initMassetManager(massetManager, basketManagerObj.basketManager.address, mAsset.address, vault.address, standardFees, {
                from: standardAccounts.default,
            });
            await setTokenProxies(mAsset, massetManager);
            await basketManagerObj.mockToken1.approve(massetManager.address, mintAmount, {
                from: standardAccounts.dummy1,
            });
            await massetManager.mint(basketManagerObj.mockToken1.address, mintAmount, {
                from: standardAccounts.dummy1,
            });
        ...
        context("should fail", () => {
            it("when bridge is not valid", async () => {
                await basketManagerObj.basketManager.setBridge(basketManagerObj.mockToken1.address, constants_1.ZERO_ADDRESS, {
                    from: standardAccounts.default,
                });
                await (0, test_helpers_1.expectRevert)(massetManager.methods["redeemToBridge(address,uint256,address)"](basketManagerObj.mockToken1.address, mintedMassets, standardAccounts.dummy2, { from: standardAccounts.dummy1 }), "VM Exception while processing transaction: reverted with reason string 'invalid bridge'");
            });
        });
        context("should succeed", () => {
            it("if all params are valid", async () => {
                const withdrawalFee = mintedMassets
                    ...
                    .div(constants_1.FEE_PRECISION);
                await massetManager.methods["redeemToBridge(address,uint256,address)"](basketManagerObj.mockToken1.address, mintedMassets, standardAccounts.dummy2, { from: standardAccounts.dummy1 });
    ...
    describe("onTokensMinted", async () => {
        ...
        context("should fail", async () => {
            it("when it's not called by bridge", async () => {
                await (0, test_helpers_1.expectRevert)(massetManager.onTokensMinted((0, tools_1.tokens)(1), basketManagerObj.mockToken1.address, web3.eth.abi.encodeParameters(["bytes"], [standardAccounts.dummy1]), { from: standardAccounts.default }), "VM Exception while processing transaction: reverted with reason string 'only bridge may call'");
            });
            it("when recipient address is invalid", async () => {
                await (0, test_helpers_1.expectRevert)(mockBridge.callOnTokensMinted(massetManager.address, (0, tools_1.tokens)(1), basketManagerObj.mockToken1.address, constants_1.ZERO_ADDRESS, {
                    from: standardAccounts.default,
                }), "VM Exception while processing transaction: reverted with reason string 'Converter: Error decoding extraData'");
            });
            it("when amount is zero", async () => {
                await (0, test_helpers_1.expectRevert)(mockBridge.callOnTokensMinted(massetManager.address, (0, tools_1.tokens)(0), basketManagerObj.mockToken1.address, standardAccounts.dummy1, {
                    from: standardAccounts.default,
                }), "VM Exception while processing transaction: reverted with reason string 'amount must be > 0'");
            });
            it("when basset is invalid", async () => {
                await (0, test_helpers_1.expectRevert)(mockBridge.callOnTokensMinted(massetManager.address, (0, tools_1.tokens)(1), standardAccounts.other, standardAccounts.dummy1, {
                    from: standardAccounts.default,
            ...
            it("when basket is out of balance", async () => {
                ...
                await (0, test_helpers_1.expectRevert)(mockBridge.callOnTokensMinted(massetManager.address, (0, tools_1.tokens)(10000), basketManagerObj.mockToken1.address, standardAccounts.dummy1, { from: standardAccounts.default }), "VM Exception while processing transaction: reverted with reason string 'basket out of balance'");
            });
        });
        context("should succeed", async () => {
            it("whit all valid params", async () => {
                const amount = (0, tools_1.tokens)(1);
                ...
                const recepit = await mockBridge.callOnTokensMinted(massetManager.address, amount, basketManagerObj.mockToken1.address, recipient, { from: standardAccounts.default });
                await test_helpers_1.expectEvent.inTransaction(recepit.tx, MassetManager, "Minted", {
    ...
    describe("redeemByBridge", async () => {
        const fees = {
            ...standardFees,
            withdrawalBridge: new tools_1.BN(0),
        };
        beforeEach(async () => {
            vault = await FeesVault.new();
            massetManager = await MassetManager.new();
            ...
            mockBridge = await MockBridge.new();
            basketManagerObj = await createBasketManager(massetManager, [18, 18], [1, 1], [mockBridge.address, mockBridge.address]);
            await initMassetManager(massetManager, basketManagerObj.basketManager.address, mAsset.address, vault.address, fees);
            await setTokenProxies(mAsset, massetManager);
        });
        context("should fail", async () => {
            it("when it's not called by bridge", async () => {
                await (0, test_helpers_1.expectRevert)(massetManager.redeemByBridge(basketManagerObj.mockToken1.address, (0, tools_1.tokens)(1), standardAccounts.dummy1, {
                    from: standardAccounts.default,
                }), "VM Exception while processing transaction: reverted with reason string 'must be called by bridge'");
            });
            it("when recipient address is invalid", async () => {
                await (0, test_helpers_1.expectRevert)(mockBridge.callRedeemByBridge(massetManager.address, basketManagerObj.mockToken1.address, (0, tools_1.tokens)(1), constants_1.ZERO_ADDRESS, {
                    from: standardAccounts.default,
                }), "VM Exception while processing transaction: reverted with reason string 'must be a valid recipient'");
            });
            it("when amount is zero", async () => {
                await (0, test_helpers_1.expectRevert)(mockBridge.callRedeemByBridge(massetManager.address, basketManagerObj.mockToken1.address, (0, tools_1.tokens)(0), standardAccounts.dummy1, {
                    from: standardAccounts.default,
                }), "VM Exception while processing transaction: reverted with reason string 'mAsset quantity must be greater than 0'");
            });
            it("when basset is invalid", async () => {
                await (0, test_helpers_1.expectRevert)(mockBridge.callRedeemByBridge(massetManager.address, standardAccounts.other, (0, tools_1.tokens)(1), standardAccounts.dummy1, {
                    from: standardAccounts.default,
                }), "VM Exception while processing transaction: reverted with reason string 'invalid basset'");
            });
            it("when basket is out of balance", async () => {
                ...
                await (0, test_helpers_1.expectRevert)(mockBridge.callRedeemByBridge(massetManager.address, basketManagerObj.mockToken1.address, (0, tools_1.tokens)(10000), standardAccounts.dummy1, { from: standardAccounts.default }), "VM Exception while processing transaction: reverted with reason string 'basset balance is not sufficient'");
            });
        });
        context("should succeed", async () => {
            it("whit all valid params", async () => {
                ...
                await basketManagerObj.mockToken1.approve(massetManager.address, initialToken1Balance, {
                    from: recipient,
                });
                await massetManager.mint(basketManagerObj.mockToken1.address, initialToken1Balance, {
                    from: recipient,
                });
                ...
                const recepit = await mockBridge.callRedeemByBridge(massetManager.address, basketManagerObj.mockToken1.address, amount, recipient, { from: standardAccounts.default });
                await test_helpers_1.expectEvent.inTransaction(recepit.tx, MassetManager, "Redeemed", {
                    redeemer: recipient,
                    recipient,
                ...
                expect(bridgeMassetBalance).bignumber.to.eq(constants_1.ZERO, "should not take any massets from bridge");
                const bridgeBassetBalance = await basketManagerObj.mockToken1.balanceOf(mockBridge.address);
                expect(bridgeBassetBalance).bignumber.to.eq(constants_1.ZERO, "bridge should not get any bassets");
                const vaultBalance = await mAsset.balanceOf(vault.address);
    ...
    describe("precision conversion", async () => {
        const basset1Factor = new tools_1.BN(100);
        const basset2Factor = new tools_1.BN(-1000000);
        beforeEach(async () => {
            vault = await FeesVault.new();
            massetManager = await MassetManager.new();
            mAsset = await createMassetToken(massetManager);
            basketManagerObj = await createBasketManager(massetManager, [20, 12], [basset1Factor, basset2Factor]);
            await initMassetManager(massetManager, basketManagerObj.basketManager.address, mAsset.address, vault.address, standardFees);
            await setTokenProxies(mAsset, massetManager);
        });
        it("works both ways", async () => {
            const amount = (0, tools_1.tokens)(10000);
            ...
            await basketManagerObj.mockToken2.approve(massetManager.address, initialToken2Balance, {
                from: standardAccounts.dummy1,
            });
            await massetManager.mint(basketManagerObj.mockToken2.address, initialToken2Balance, {
                from: standardAccounts.dummy1,
            });
            const fee = amount.mul(standardFees.deposit).div(constants_1.FEE_PRECISION);
            const account1BalanceAfterMint = amount.sub(fee).mul(new tools_1.BN(2));
            expect(await getBalance(mAsset, standardAccounts.dummy1)).bignumber.to.equal(account1BalanceAfterMint);
            expect(await getBalance(basketManagerObj.mockToken1, standardAccounts.dummy1)).bignumber.to.equal((0, tools_1.tokens)(0));
            expect(await getBalance(basketManagerObj.mockToken2, standardAccounts.dummy1)).bignumber.to.equal((0, tools_1.tokens)(0));
            await mAsset.transfer(standardAccounts.dummy2, amount, {
                from: standardAccounts.dummy1,
            });
            expect(await getBalance(mAsset, standardAccounts.dummy1)).bignumber.to.equal(account1BalanceAfterMint.sub(amount));
            ...
            expect(await getBalance(mAsset, standardAccounts.dummy2)).bignumber.to.equal((0, tools_1.tokens)(0));
            ...
            const totalFee = fee.mul(new tools_1.BN(2)).add(withdrawalFee); // 2 mints and one redeem
            const vaultBalance = await mAsset.balanceOf(vault.address);
            expect(vaultBalance).bignumber.to.eq(totalFee);
        });
    });
});
const zeroBridges = [constants_1.ZERO_ADDRESS, constants_1.ZERO_ADDRESS];
async function initMassetManager(massetManager, basketManagerAddress, tokenAddress, vaultAddress, fees, txDetails = { from: standardAccounts.default }) {
    ...
    await massetManager.initialize(basketManagerAddress, tokenAddress, false, txDetails);
    await massetManager.upgradeToV3(basketManagerAddress, tokenAddress, vaultAddress, feesManager.address, txDetails);
}
async function createBasketManager(massetManager, decimals, factors, bridges = zeroBridges) {
...
async function createMassetToken(massetManager) {
    const mAsset = await MetaAssetToken.new("Mock1", "MK1");
    return mAsset;
}
async function setTokenProxies(mAsset, massetManager) {
...
async function getBalance(mAsset, who) {
    return mAsset.balanceOf(who);
}
//# sourceMappingURL=TestMassetManager.spec.js.map